...
class FairScanGradingSystem:
    ...
    def _load_models(self):
        ...
        try:
            gpu_layers = 35 if self.use_gpu and torch.cuda.is_available() else 0
    ...
    {ontology_context}{additional_context}
    ...
    EVALUATION TASK:
    Score this resume against ONLY the criteria listed below. Use evidence-based scoring with professional precision.
    ...
    UNIVERSAL SCORING THRESHOLDS (EVIDENCE-BASED):
    * 90-100: Extensive experience + leadership/senior roles + significant achievements + expert-level demonstration
    * 80-89: Solid experience + clear competency + specific accomplishments + some advanced responsibilities
    * 70-79: Good experience + demonstrated capability + concrete examples + practical application
    * 60-69: Basic experience + some evidence + fundamental understanding + limited examples
    * 50-59: Minimal mentions + limited evidence + basic familiarity
    * <50: Little to no relevant evidence or experience
    ...
    {{
        ...
        "evaluation_summary": "One sentence professional assessment focusing on key strengths and role fit",
        ...
        "CONTINUE GENERATING UNTIL YOU HAVE COMPLETED ALL SECTIONS OF THE RESPONSE."
            ...
            if json_match:
                try:
                    ...
                    return {"error": "Invalid JSON response", "raw_response": generated_text[:1000]}
            else:
                return {"error": "No JSON found in response", "raw_response": generated_text[:1000]}
    ...
    def _generate_criteria_template(self, criteria):
        ...
        for criterion in criteria:
            template = f'''"{criterion.name}": {{
                "score": <0-100>,
                "evidence_found": ["specific quotes and examples from resume demonstrating this criteria"],
