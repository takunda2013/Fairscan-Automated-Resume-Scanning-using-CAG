...
#                 # Default: treat as description - THIS IS THE KEY FIX
#                 if len(line.strip()) > 0:
#                     criterion["_current_field"] = "description"
...
#     def _should_continue_job_description(self, line: str) -> bool:
#         """Determine if a line should continue the job description"""
#         # Don't continue if it's a field definition
#         if any(x in line.lower() for x in ['description:', 'importance:', 'keywords:', 'weight:']):
#             return False
        
#         # Don't continue if it's clearly a section header
#         if line.upper() in ["HIRING CRITERIA", "--- DIVIDER ---"]:
#             return False
        
#         # Don't continue if it looks like a divider
#         if re.match(r'^[-=*_]{5,}$', line):
#             return False
        
#         # Don't continue if it's a criterion title
#         if self._is_criterion_title(line):
#             return False
        
#         # Don't continue if it's a new job title
#         if line.startswith("Job Title:") or line.startswith("Title:"):
#             return False
        
#         # Continue if it's regular text
#         return True
...
#                 # Continue job description if flag is set and line qualifies
#                 if (job_description_continues and 
#                     self._should_continue_job_description(line)):
...
#                 # If we get here and job_description_continues is True, 
#                 # but the line didn't qualify, stop continuing
#                 job_description_continues = False
...
class EnhancedOntologyConverter:
    ...
    def _parse_criterion_line(self, line: str, criterion: Dict):
        ...
        if field_match:
            ...
            if not current_field:
                ...
                # This is the key fix - be more liberal about treating lines as descriptions
                if len(line.strip()) > 0:  # Any non-empty line can be a description
                    criterion["_current_field"] = "description"
                    current_field = "description"
                    # Don't return here, let the code below handle it
    ...
    def parse_template_structure(self, text: str) -> Dict:
        ...
        job_mode = False
        current_job = None
        current_criteria = None  # Changed from last_criteria to current_criteria
        job_description_continues = False
        ...
        for line in lines:
            ...
            if line.startswith("Job Title:") or line.startswith("Title:"):
                ...
                job_mode = True
                job_description_continues = False
                current_criteria = None  # Reset criteria
                continue
            ...
            # HIRING CRITERIA SECTION
            if job_mode and current_job and line.upper() == "HIRING CRITERIA":
                job_description_continues = False  # Stop continuing job description
                continue
            ...
            if job_mode and current_job:
                # Check if we're continuing a job description
                if (job_description_continues and 
                    not any(x in line.lower() for x in ['description:', 'importance:', 'keywords:', 'weight:']) and 
                    not self._is_criterion_title(line) and
                    line.upper() != "HIRING CRITERIA"):  # Stop at HIRING CRITERIA
